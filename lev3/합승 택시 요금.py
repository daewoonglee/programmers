...
def solution(n, s, a, b, fares):
    ...
    def floyd_warshall():
        for z in range(n):
            for i in range(n):
                # if i == z: continue # 로직상 if문 없어도 제대로 돔, if문 비교 횟수 감소가 더 나음
                for j in range(i+1):
                    fare_maps[i][j] = fare_maps[j][i]
                for j in range(i+1, n):
                    fare_maps[i][j] = min(fare_maps[i][j], fare_maps[i][z] + fare_maps[z][j])

        return fare_maps

    # # dijkstra - 0.49168040200000007
    # fare_dict = {k:[] for k in range(n)}
    # for data in fares:
    #     start, end, fare = data
    #     fare_dict[start-1].append([end-1, fare]) # 1~n시작을 0~n-1로 변환
    #     fare_dict[end-1].append([start-1, fare])
    # fare_maps = [dijkstra(i) for i in range(n)]

    # floyd warshall - 1455.153891
    fare_maps = [[float("inf")] * n for _ in range(n)]
    for i, j, fare in fares:
        fare_maps[i-1][j-1] = fare_maps[j-1][i-1] = fare
    for i in range(n):
        fare_maps[i][i] = 0
    fare_maps = floyd_warshall()

    ans = fare_maps[s-1][a-1] + fare_maps[s-1][b-1]
    for i in range(n):
        fare_way = fare_maps[s-1][i] + fare_maps[i][a-1] + fare_maps[i][b-1]
        if fare_way < ans:
            ans = fare_way
    return ans
