...
def solution(files):
    # 0.182337852
    info = [[i, 0, 0] for i in range(len(files))]
    for i, file in enumerate(files):
        ...
        for f in file:
            if 48 <= ord(f) <= 57:
                ...
                break
        info[i][1:] = head, int(number)
    info.sort(key=lambda x: (x[1].lower(), x[-1]))
    return [files[i[0]] for i in info]
...
if __name__ == '__main__':
    ...
    query = [["img12.png", "img10.png", "img02.png", "img1.png", "IMG01.GIF", "img2.JPG"],
