...
def solution(expression):
    # 1.2938152
    # tokenizer = re.compile('(\d+|[^ 0-9])')
    # split_list = re.findall(tokenizer, expression)
    # oper = list(set([s for s in split_list if not s.isdigit()]))
    # p = list(itertools.permutations(range(len(oper)), len(oper)))
    # sum_list = []
    # for idx in p:
    #     s_list = split_list.copy()
    #     for i in idx:
    #         while oper[i] in s_list:
    #             j = s_list.index(oper[i])
    #             value = str(eval(''.join([s_list.pop(j - 1) for _ in range(3)])))
    #             s_list.insert(j - 1, value)
    #     sum_list.append(abs(int(s_list[0])))
    # return max(sum_list)

    # 1.03021295
    split_list = re.split(r'(\D)', expression)
    oper = [x for x in ['*', '+', '-'] if x in expression]
    p = list(itertools.permutations(oper))
    ...
    for idx in p:
        ...
        for i in idx:
            while i in s_list:
                j = s_list.index(i)
                s_list[j - 1] = str(eval(s_list[j - 1] + s_list[j] + s_list[j + 1]))
                s_list = s_list[:j] + s_list[j + 2:]
        answers.append(abs(int(s_list[0])))
    return max(answers)
...
import timeit
avg_time = 0.
tests = ["100-200*300-500+20", "50*6-3*2"]
for t in tests:
